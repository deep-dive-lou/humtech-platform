...
from .storage import SPACES_BUCKET, head_object, presign_get, presign_put, upload_object
...
@router.post("/r/{token}/items/{item_id}/upload")
async def upload_file(
    ...
    """Upload a file through the server to Spaces (no client-side CORS needed)."""
    token_hash = _hash_token(token)
    ...
    item = await conn.fetchrow("""
        SELECT id, tenant_id FROM portal.doc_request_items
    ...
    if content_type not in ALLOWED_MIME_TYPES:
        raise HTTPException(
            status_code=400,
            detail=f"File type not allowed. Accepted: PDF, JPEG, PNG, HEIC, TIFF, DOCX, DOC.",
        )
    ...
    # -- Record in DB (single transaction) --------------------------------
    async with conn.transaction():
        ...
        """, tenant_id, request_id, item["id"], "client_upload",
            ...
            filename, content_type, size_bytes, "clean", "client", None)
...
@router.post("/r/{token}/items/{item_id}/zones/save-draft")
async def save_zone_draft(
    ...
    """Auto-save a single zone value without marking the item as uploaded."""
    token_hash = _hash_token(token)
    ...
    sig_key = body.get("signature_file_key")
    ...
    if sig_key:
        await conn.execute(
            ...
            sig_key, zone_id, item_id, tok["tenant_id"],
        )
